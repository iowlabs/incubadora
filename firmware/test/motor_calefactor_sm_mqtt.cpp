...
#define PID_OUTPUT_PIN 25  // Pin para la salida del PID (control)
#define IN1 33
#define IN2 32
...
double Kp = 30, Ki = 5, Kd = 10;  // Valores para el PID
...
void setup()
{
	...
	//Inicializar el driver de HEATER
	...
	ledcAttachPin(PID_OUTPUT_PIN, 0);  // Asociar el pin ENA al canal 0
	ledcWrite(0, 200);
...
void handle_heater()
{
  	...
  	if (currentMillis - lastTempRead >= TEMP_INTERVAL)
	{
    	...
    	//sensors.requestTemperatures();  // Solicita la temperatura del sensor
    	//temperature = sensors.getTempCByIndex(0);  // Obtiene la lectura de temperatura
    	temperature = bmp.readTemperature();  // Obtiene la lectura de temperatura
  	...
  	if (currentMillis - lastPIDRead >= PID_INTERVAL)
	{
    	...
    	// Enviar la señal de control al pin de salida del PID
    	//analogWrite(PID_OUTPUT_PIN, Output);
		ledcWrite(PID_OUTPUT_PIN, Output);

    	//Imprimir la salida del PID en la terminal
    	Serial.print("Salida PID (Pin 25): ");
    	Serial.println(Output);
  	}
}
